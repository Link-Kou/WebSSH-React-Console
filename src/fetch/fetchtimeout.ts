...
import {utilsObject} from 'utilscommon';
import {OPSMsg} from '@intl';
 ...
 * @constructor
 */
export function ReqError(msg: string | Function | React.ReactElement): any {
    return {
        code: -10000,
        msg: msg,
        data: null,
    ...
    Promise.race(
        ...
        .then((fetchreq: any) => {
            try {
                ...
                callback(ReqError(OPSMsg('OPS_ServerError')))
            }
        })
        .catch((e) => {
            //请求失败
            callback(ReqError(OPSMsg('OPS_FetchTimeout')))
        });
}
...
export function Fetch(url: string, model: any, method: string = 'POST', headers: any = {'Content-Type': 'application/json'}) {
    ...
    return (resolve: any, callerror: any) => {
        const body = model ? {body: JSON.stringify(model)} : {}
        return fetch(url, {
            method,
            headers,
            ...body
            ...
            .catch((error) => {
                if (error.message === 'request error') {
                    resolve(ReqError(OPSMsg('OPS_FetchError')))
                } else {
                    //请求失败
                    resolve(ReqError(OPSMsg('OPS_ClientError')))
                }
            })
    }
}
